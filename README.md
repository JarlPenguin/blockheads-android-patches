...
> **Notes from Claude:**
> **Screen profile.** Three defects stack. `-[UIDevice userInterfaceIdiom]` resolves through a `dispatch_once` block to `getenv("TABLET")`, which nothing in the APK ever sets - so every device reports `UIUserInterfaceIdiomPhone`, and that accessor gates UIKit's whole `~ipad` resource resolver, leaving the shipped iPad nib and launch images unreachable. `-[UIScreen preferredMode]` requests `emulatedMode:12`, which computes `scale = VerdePluginNativeWidth() / 600.0` - a constant unrelated to density. That mode survives ~120 ms before `main` calls `setCurrentMode:` with `emulatedMode:11`, which snaps the device to a fixed iOS profile by aspect ratio and pixel thresholds; both classes can be the live instance, hence both are overridden. Since `bounds` is `currentMode.size / scale` and `PIXEL_SCALE` caches `UIScreen.scale` in `-[EAGLView initWithCoder:]`, one bad mode poisons layout, framebuffer sizing and touch mapping together - a 2560×1600 tablet landed on a 414×736 @3.0 iPhone 6 Plus profile. Correcting geometry alone isn't enough: `MainMenuUI` and the `render:translation:pinchScale:` family branch on a literal `415.0`-point width, so a large phone crosses the line and gets tablet layout.
>
> **Orientation.** Apportable builds its view hierarchy on a dedicated `MainThread` while Android delivers config changes on `main`, so rotation reaches `ViewRootImpl.checkThread` and throws `CalledFromWrongThreadException` - device-independent in mechanism, but only observed on One UI. Separately, `-[EvolutionViewController shouldAutorotate]` returns `NO` under Android's rotation lock, gating `-[UIDevice _setOrientation:changed:]` - gating `-[UIDevice _setOrientation:changed:]` - the engine's normal route to `_platform_setOrientation:`, through which every post-launch write of the global behind `statusBarOrientation` passes. `-[World acceleration:]` reads that global every sample and applies its axis transforms only for orientations 2 and 4, so a locked session leaves tilt stuck in the LandscapeLeft frame. The value can't simply be corrected: the layout path reads it too, and setting it reconfigures the render surface for a window that never rotated. Third, `OrientationEventListener` reports `ORIENTATION_UNKNOWN` when the device is flat and `SplashScreen$1` filters it, so a flat launch never corrects the engine's hardcoded startup `_setOrientation:4` and never pins the activity - the window follows the device while the game doesn't.
>
> **Multi-window.** `_applyMode` pushes `currentMode.size` to `SurfaceHolder.setFixedSize`, so SurfaceFlinger stretches a fixed buffer over the pane rather than relaying out. Nothing tells the engine the window changed - `AndroidConfigurationChanged`, `nativeOnSurfaceChanged` and `GLSurfaceRenderer.surfaceChanged` all cross JNI carrying no dimensions. The transpose in `_applyMode` keys on `-[UIDevice orientation]`, which describes the device, not the pane. The engine also saves `_platform_getOrientation` on `WillResignActive` and replays it through `_platform_setOrientation:` on `WillEnterForeground`, bypassing `_setOrientation:changed:` on the assumption that orientation can't change while backgrounded - true in fullscreen, where the activity pin holds. Entering a split stops and restarts the activity while Android forces the pane into a new orientation, so with rotation lock on the replay rewrites `statusBarOrientation` to the pre-split value after the kick has corrected it. Geometry is unaffected, since `_applyMode` takes its transpose from the overridden `-[UIDevice orientation]`; the game's own dialogs, which `BlockAlertView` builds from `statusBarOrientation` at construction, come up in the old orientation.
>
> **One UI specifics.** On three transitions - dragging the divider to exit a split, entering freeform from recents, and rotating while backgrounded - the window is displayed while the activity remains stopped: `onConfigurationChanged` arrives with no `onStart`, no traversal runs, and the hierarchy keeps pre-transition bounds indefinitely. Nothing in-process can lay out a stopped activity, which is why `requestLayout`, `setAttributes`, system-UI toggles, `SurfaceView` visibility toggles and `moveTaskToFront` all fail; only a genuine stop/start recovers it. Separately, the game renders into a `Dialog` (`Window$WindowDialog`, held in `Window.sKeyWindowDialog`), not the activity window - `Activity.getWindow()` returns a transparent `BASE_APPLICATION` window that draws nothing. Swapping panes moves the window without resizing it, the dialog's window keeps requesting the old height, and the 94 px difference composites as a black strip with touches offset to match.

**Screen profile**

* Overrides `-[UIScreenMode scale]` and `size`, and the same pair on `UINativeScreenMode`, which declares its own. Both are plain ivar reads, so the parent implementation remains a valid fallback. `bounds`, `PIXEL_SCALE`, `EAGLView.framebufferWidth`/`Height` and `World.tapViewport` all derive from these two and stay consistent.
* Scale is `shortEdgePixels / shortEdgeDp`, both from `Display.getRealMetrics()` and exported via `LibraryLoader.setenv`. The display, not the window, is the reference, so launching into a split doesn't skew the session. Android dp and iOS points share the 160-per-inch baseline, so the mapping is physical rather than tuned: a Tab S9 resolves to 753×1205 points.
* Reports size portrait-canonically. iOS fixes `UIScreen` to portrait and rotates the window above it; reporting live orientation produced a doubly-rotated, clipped landscape layout.
* Caps phones at 414 points so a device that would cross the internal `415.0` threshold binds to the largest phone layout the game can render. Smaller phones are untouched.
* Computes geometry lazily on first accessor call - `VerdePluginNativeWidth`/`Height` return zero until the GL surface exists. Latches on first success; the scale is never recomputed, since a mid-session change would desync touch mapping from rendering.
* Sets `TABLET` from `shortEdgeDp >= 600`, before `LibraryLoader.init()` and therefore before the `dispatch_once` reads it.
* Replaces `SplashScreen.getSplashScreen`, a short-circuit chain that tested `Default-568h@2x.png` first and so never reached any other candidate. Selection is now by idiom, scale and orientation, aspect-first within each column. Landscape borrows the iPad art on every device: iOS shipped no landscape iPhone launch image, so there's no upstream behaviour to match.
* Factors the letterbox draw into `applyLetterbox` and re-runs it from `reselect()`, hooked to `onConfigurationChanged`, so a rotation during the splash doesn't keep the launch-orientation image. No-op when the pick is unchanged, and wrapped so failure leaves the existing splash.

**Orientation**

* Catches `CalledFromWrongThreadException` on a nested `Looper.loop()`, keyed to the exact class, `checkThread` as frame 0, and an `ActivityThread.handleActivityConfigurationChanged*` frame, rate-limited to 8 per 2000 ms. `checkThread` is the first statement in `requestLayout`, so nothing has mutated; the aborted work is one layout pass on a hierarchy that is a single GL surface.
* Substitutes the window's orientation for the engine's hardcoded startup `_setOrientation:4` at `-[UIDevice _setOrientation:changed:]` - upstream of `setStatusBarOrientation:`, the Java rotation request, and `_applyMode`. One-shot, so a later user rotation passes through untouched.
* Limits `SplashScreen$1` to one orientation dispatch while the splash is up.
* Substitutes the display's orientation for `statusBarOrientation`, but only inside `-[World acceleration:]`. Fires when the real value is absent or disagrees on axis; agreement leaves it alone, preserving the engine's handedness. Substituting only on `0` doesn't survive multi-window, where a forced rotation moves the window while the lock freezes the global.
* Derives display orientation from `Display.getRotation()` cross-checked against `getRealMetrics()`, latching natural orientation on the first consistent reading. `Configuration.orientation` is not used: it describes the window in multi-window.
* Re-drives one rotation once the window geometry is real. The engine builds its UI while the window is still 0x0 and `keyWindow` is nil, so containers sized off `UIScreen.bounds` get portrait-canonical frames - on a landscape launch the game's own dialogs render anchored bottom-left with a partial dim overlay. `relayout()` runs ~250 ms later but only fixes the `UIWindow` and root view. The kick calls `_platform_setOrientation:` directly rather than `_setOrientation:changed:`: the wrapper gates on the supported-orientations mask and `shouldAutorotateToInterfaceOrientation:`, so it is vetoed outright under rotation lock, and early-outs when the requested value matches `UIDevice._orientation`. The direct call skips both; the ivar is written explicitly to keep `UIDevice.orientation` consistent. Deferred until `relayout()` succeeds, since on some launches the first pin precedes `keyWindow`, with a retry dispatched from the settle check. Re-armed on a display-orientation change in multi-window, where a forced rotation leaves the engine laid out against a stale value.
* Removes `screenOrientation="portrait"` from `VerdeActivity`; adds `screenOrientation="behind"` and `screenSize` to `BlockheadsWebView`. `behind` inherits the game's current orientation rather than the device's rotation lock. `screenSize` is defensive - the activity declares `orientation` but not `screenSize` in `configChanges`, which at `targetSdk` 27 should mean rotation recreates it, and observably doesn't.

**Multi-window**

* Adds `WindowState`, a shared Java pump owning display orientation, rotation-lock state, multi-window state and the rotation-to-`UIInterfaceOrientation` mapping. Both libraries consume it and neither depends on the other; every native call is guarded.
* Pushes window pixels on every layout pass, then re-pins via `_applyMode` and sets the `UIWindow` frame explicitly. `setCurrentMode:` is not used: `UIScreenMode` has no `isEqual:`, so its guard is pointer identity. The frame takes the window-shaped rect, since nothing here performs UIKit's window rotation.
* Measures from the raw decor view in fullscreen and subtracts `getInsets(systemBars())` only in multi-window. The window's `0x1706` flags lay the decor out as if the bars were hidden, so the fullscreen subtraction was always redundant - and it fired spuriously when the IME transition changed the reported nav-bar inset, shifting the UI scale by 23 points.
* Overrides `-[UIDevice orientation]` to answer from the pane's shape in multi-window and the display otherwise. Deriving from the window in fullscreen oscillates.
* Suppresses the activity pin in multi-window and resynchronises the engine's orientation on leaving a split, on thread 1 - the chain reaches `_applyMode`, which doesn't return from the Android UI thread.
* Registers `addOnLayoutChangeListener` alongside the existing `OnGlobalLayoutListener`. The latter stays: it is the only thing keeping display orientation and rotation-lock state current, neither of which depends on bounds.
* Detects the stopped-while-visible state via a flag set in `onStop`, and recovers by launching `Trampoline`, an empty activity with its own task affinity that finishes in `onCreate`. `onStart` arrives ahead of the trampoline's own `onCreate`, so the transition isn't visible. Wrapped, since losing it to an exception would take the settle check's re-measure with it.
* Re-sends `sKeyWindowDialog`'s window its own `LayoutParams` when `WindowMetrics.getBounds().height()` changes while the decor's measured size does not - the signature of a pane swap. `requestLayout()` on that window blanks it; the params re-send is what makes WindowManager re-issue the height. Insets are unusable as the trigger: they read zero on both sides of the swap.
