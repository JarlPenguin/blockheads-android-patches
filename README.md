...
> **Notes from Claude:**
> **Screen profile.** Three independent defects stack here. `-[UIDevice userInterfaceIdiom]` resolves through a `dispatch_once` block to `getenv("TABLET")`, matched case-insensitively against `yes`/`true`/`1` - there is no device detection at all, and nothing in the shipped APK ever sets the variable, so every Android device reports `UIUserInterfaceIdiomPhone`. That single accessor gates both `NSMainNibFile~ipad` and UIKit's whole `~ipad`/`@2x` resource-suffix resolver, so `MainWindow-iPad.nib` and the iPad launch images ship in the APK and are never once consulted. Separately, `-[UIScreen preferredMode]` requests `+[UIScreenMode emulatedMode:12]`, which forwards to `+[UINativeScreenMode nativeMode:]` - and that method discards its argument and computes `scale = VerdePluginNativeWidth() / 600.0`, a magic constant with no relation to display density. That mode survives about 120 ms: `main` then calls `setCurrentMode:` with `+[UIScreenMode emulatedMode:11]`, which reads the device's native dimensions and hands them to `+[UIScreenMode bestEmulatedMode:]` - a catalogue of fixed iOS device profiles, bucketed by aspect ratio and then snapped down by pixel thresholds. So the session runs on whichever iPhone the device snaps to rather than on the `/600` profile, and both classes can be the live instance, which is why both are overridden. `-[UIScreen bounds]` is `currentMode.size / currentMode.scale` and `PIXEL_SCALE` is a copy of `UIScreen.scale` cached in `-[EAGLView initWithCoder:]`, so one bad mode poisons layout, framebuffer sizing and touch coordinate mapping together; on a 2560×1600 tablet the result was a hardcoded 414×736 @3.0 iPhone 6 Plus profile. Finally the game itself branches on a literal `415.0`-point width at every layout site in `MainMenuUI` and the `render:translation:pinchScale:` family to choose phone versus tablet layout, so correcting the geometry is not sufficient on its own - a large phone lands just over the line and gets inventory-on-the-right and a D-pad in portrait.
>
> **Orientation.** Apportable creates its view hierarchy on a dedicated `MainThread`, but Android delivers configuration changes on `main`. On rotation, `ActivityThread.handleActivityConfigurationChanged` → `ViewRootImpl.updateConfiguration` → `requestLayout` → `checkThread` compares the two and throws `CalledFromWrongThreadException`. The mechanism is device-independent; One UI appears to force a layout pass where AOSP returns early, so it only manifests on Samsung. Separately, `-[EvolutionViewController shouldAutorotate]` returns `NO` whenever Android's `accelerometer_rotation` setting is `0`. That gates `-[UIDevice _setOrientation:changed:]`, the only path that calls `_platform_setOrientation:`, the only writer of the global behind `-[UIApplication statusBarOrientation]`. `-[World acceleration:]` reads that global every accelerometer sample and applies its axis transforms only for orientations 2 and 4, so with rotation locked it stays `0`, both transforms are skipped, and tilt is stuck in the LandscapeLeft frame regardless of what the display shows. The value can't simply be corrected: `-[UIScreen _orientationChanged:]` reads `UIDevice.orientation` and the layout path reads `statusBarOrientation`, so setting either correctly reconfigures the render surface for a window that never rotated. A third failure hinges on `OrientationEventListener` reporting `ORIENTATION_UNKNOWN` when the device is near-horizontal. `SplashScreen$1` filters that out, so launching flat means no orientation is ever dispatched: the engine's hardcoded startup `_setOrientation:4` is never corrected, and the activity is never pinned via `_setRequestedOrientation`. The engine renders landscape in a portrait window, and because the activity stays `unspecified`, Android rotates it directly on sensor input - bypassing `-[World allowsRotation]`, which otherwise vetoes rotation while `requiresMotionEvents` is true. The window follows the device while the game doesn't.
>
> **Multi-window.** Split-screen compounds both. `-[UIScreen _applyMode]` pushes `currentMode.size` through the bridged `setContentWidth:height:` to `SurfaceHolder.setFixedSize`, pinning the framebuffer; SurfaceFlinger then stretches that fixed buffer over whatever the window actually is, so a split pane renders warped rather than relaid-out. Nothing tells the engine the window changed: `AndroidConfigurationChanged`, `nativeOnSurfaceChanged` and `GLSurfaceRenderer.surfaceChanged` all cross JNI carrying no dimensions, and the last resolves to a selector no binary implements. The transpose in `_applyMode` keys on `-[UIDevice orientation]`, which describes the device rather than the pane - in a split the two are decoupled, and a phone held upright can host a landscape window. Separately, a pinned activity is letterboxed rather than filling its pane, so the engine's rotation veto - expressed by declining to call `setRequestedOrientation` - cannot be honoured there.
>
> Two further defects surface only on Samsung's One UI. On three transitions - dragging the divider to exit a split, entering freeform from recents, and rotating while backgrounded - the window is displayed while the activity remains stopped: `onConfigurationChanged` arrives with no subsequent `onStart`, so no traversal runs and the hierarchy keeps the pre-transition bounds indefinitely. Nothing in-process can lay out a stopped activity, which is why `requestLayout`, `setAttributes`, system-UI toggles, `SurfaceView` visibility toggles and `moveTaskToFront` all leave it unchanged; only a genuine stop/start recovers it. Separately, the game renders into a `Dialog` (`Window$WindowDialog`, held in the static `Window.sKeyWindowDialog`), not into the activity window - `Activity.getWindow()` returns a transparent `BASE_APPLICATION` window that draws nothing. Swapping panes in a split moves the window without resizing it; the dialog's window keeps requesting the old height, and the 94 px difference composites as a black strip at the top with every touch offset to match.
...
* Computes scale as `shortEdgePixels / shortEdgeDp`, both read from `Display.getRealMetrics()` and exported from `LibraryManager.initializeLibv` through Apportable's own `LibraryLoader.setenv` bridge. The display rather than the window is the reference, so launching into a split does not permanently skew the session. Android dp and iOS points share the same 160-per-inch baseline, so the mapping is physically correct rather than a tuned constant: a Galaxy Tab S9 resolves to 753×1205 points, close to an iPad's 768×1024.
...
* Computes geometry lazily on first accessor call rather than at load. `VerdePluginNativeWidth`/`Height` return zero until the GL surface exists, which is well after `BackgroundLibraryLoader$1` runs; the value latches on first success and the scale is never recomputed, since `PIXEL_SCALE` is cached once and a mid-session change would desync touch mapping from rendering.
* Sets `TABLET` from the same `shortEdgeDp >= 600` test, restoring the idiom the env-var mechanism was designed to carry. Written before `LibraryLoader.init()` and therefore before the ObjC runtime's `dispatch_once` reads it, so the intended path is used rather than patching the accessor.
...
* Substitutes the display's orientation for `statusBarOrientation`, but only inside `-[World acceleration:]`, so the layout path still sees the real value. The substitution fires when the real value is absent (`0`) or disagrees on axis with the display's. Agreement on axis leaves it untouched, preserving the engine's own handedness, which the tilt transforms were written against. The original patch substituted only on `0`, on the argument that the value is absent-or-correct and never stale; that does not survive multi-window, where a forced rotation moves the window while the rotation lock keeps the global frozen at its old value.
* Derives the display orientation from `Display.getRotation()` cross-checked against `getRealMetrics()`, latching the device's natural orientation on the first consistent reading and holding the previous value when the two disagree mid-transition. `Configuration.orientation` is not used: it describes the window in multi-window and would misfire there. The same value seeds `_setRequestedOrientation` at launch, covering the flat-launch case where no sensor dispatch occurs.
* Removes `screenOrientation="portrait"` from `VerdeActivity` so the game can launch directly in landscape.
* Adds `screenOrientation="behind"` and `screenSize` to the `BlockheadsWebView` activity's entry. `behind` makes it inherit `VerdeActivity`'s current orientation instead of resolving `unspecified` to the device's rotation lock. `screenSize` is defensive: the activity declares `orientation` in `configChanges` but not `screenSize`, which at `targetSdk` 27 should mean rotation destroys and recreates it - but it observably doesn't, and the reason hasn't been established. Declaring `screenSize` makes the intended behaviour explicit rather than relying on whatever is currently absorbing the change.

**Multi-window**

* Adds `WindowState`, a shared Java pump owning display orientation, rotation-lock state, multi-window state and the rotation-to-`UIInterfaceOrientation` mapping. Both libraries consume it and neither depends on the other; every native call is guarded, so a patch set carrying only one library degrades rather than throws.
* Pushes the window's own pixels down on every layout pass, measured from the decor view with system-bar insets subtracted, then re-pins the surface via `_applyMode` and updates the `UIWindow` frame explicitly. `setCurrentMode:` is not used: `UIScreenMode` has no `isEqual:` override, so its guard is pointer identity and re-sending the same object is silently dropped. The frame takes the window-shaped rect rather than the portrait-canonical bounds, since nothing here performs UIKit's window rotation.
* Overrides `-[UIDevice orientation]` to answer from the pane's shape in multi-window and from the display otherwise. Deriving it from the window in fullscreen oscillates, since the window is itself downstream of the rotation.
* Suppresses the activity pin while in multi-window, and resynchronises the engine's orientation on leaving a split - on thread 1, because the call chain reaches `_applyMode`, which does not return when invoked from the Android UI thread.
* Extends `Lifecycle.expectGainFocus`'s rescue to fire in multi-window as well as on a missing unfocus event. Entering a split stops and restarts the activity, throttling the `CADisplayLink` to `frameInterval=30` and invalidating its timer; `...DidBecomeActive` never posts because `sWindowHasFocus` stays false while the other pane holds focus, so nothing calls `_startTimer` until a tap delivers genuine focus. A visible split-screen app is foreground regardless of focus.
* Registers `addOnLayoutChangeListener` on the decor view alongside the existing `OnGlobalLayoutListener`. The global-layout listener stays because it is the only thing keeping display orientation and the rotation-lock setting current - neither depends on bounds, and a 180° rotation or a letterboxed pin changes rotation with the decor's size unchanged. `sLastW`/`sLastH` dedupe the overlap.
* Detects the stopped-while-visible state via a `sStopped` flag set in `onStop` and cleared in `onStart`, and recovers by launching `Trampoline` - an empty activity with its own task affinity that finishes in `onCreate`. The launch is enough: `onStart` arrives ahead of the trampoline's own onCreate, so the transition is not visible. The launch is wrapped, since losing it to an exception would take the settle check's re-measure with it.
* Re-sends `sKeyWindowDialog`'s window its own existing `LayoutParams` when `WindowMetrics.getBounds().height()` changes while the decor's measured size does not, which is the signature of a pane swap. `requestLayout()` on the same window blanks it; the params re-send is what makes WindowManager re-issue the height. Insets are not usable as the trigger - `getInsets(systemBars())` reports zero on both sides of the swap, and the height difference appears only in `WindowMetrics`.

_Known limitations: In split-screen the in-game tilt-control orientation lock does not hold when auto-rotation is enabled - Android does not honour a fixed orientation in multi-window, so the pin the engine's veto relies on cannot be kept; the tilt frame re-bases correctly and the controls stay usable._
